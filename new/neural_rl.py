...
from kessler_game.new.Scenarios_ import ring_static_top
...
state = env.reset(scenario=ring_static_top, controllers=[Controller_neuro])
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            print("Network output shape:", output)
            return output
    else:
        print("Random action:", torch.tensor([env.action_space.sample()], device=device, dtype=torch.float32))
...
for i_episode in range(num_episodes):
    ...
    state = env.reset(controllers=[Controller_neuro], scenario=ring_static_top)
    ...
    for t in itertools.count():
        ...
        observation, reward, terminated, truncated, _ = env.run_step(action)
